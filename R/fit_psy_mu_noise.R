fit_psy_mu_noise <- function(x_p,
  ...
  if (regularization) {
    ...
    nll <- create_nll_psy_mu_noise(x_p, x_c, y, fun, fun_mu, fun_sigma)
  }
  ...
  if (full) {
    ...
    psy <- x_lim |>
      ...
      mutate(prop = fun(x_p, x_c, fun_mu, fun_sigma, par$par))
    
    mu <- tibble(x = seq(0.00, 0.14, length = 400)) |> #seq(min(x_c), max(x_c), length = 401)) |>
      mutate(y = fun_mu(x, par$par),
             sigma = fun_sigma(x, par$par)/sqrt(2),
